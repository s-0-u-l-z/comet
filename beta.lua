...
local function applyCorner(parent, rounded, radius)
	if rounded then
...
local function applyBorder(frame, opts, defaultColor)
	local conf = opts or {}
	local enabled = conf.border
	if enabled == nil then enabled = true end
	local function parseColor(v, fallback)
		if typeof(v) == "Color3" then return v end
		if type(v) == "string" then
			local ok, res = pcall(function() return utility.from_hex(v) end)
			if ok and res then return res end
			local r,g,b,a = v:match("rgba?%((%d+),%s*(%d+),%s*(%d+)")
			if r and g and b then
				return Color3.fromRGB(tonumber(r),tonumber(g),tonumber(b))
			end
		end
		return fallback or Color3.fromRGB(12,12,12)
	end
	local color = parseColor(conf.borderColor, defaultColor or Color3.fromRGB(12,12,12))
	local thickness = conf.borderThickness or 1
	local top = (conf["border-top"] ~= nil) and conf["border-top"] or (conf.borderTop ~= nil and conf.borderTop or enabled)
	local bottom = (conf["border-bottom"] ~= nil) and conf["border-bottom"] or (conf.borderBottom ~= nil and conf.borderBottom or enabled)
	local left = (conf["border-left"] ~= nil) and conf["border-left"] or (conf.borderLeft ~= nil and conf.borderLeft or enabled)
	local right = (conf["border-right"] ~= nil) and conf["border-right"] or (conf.borderRight ~= nil and conf.borderRight or enabled)
	for _,n in ipairs({"_bTop","_bBottom","_bLeft","_bRight"}) do
		local ex = frame:FindFirstChild(n)
		if ex then ex:Destroy() end
	end
	if not enabled then return end
	if top then
		utility.new("Frame",{Name="_bTop",BackgroundColor3=color,BorderSizePixel=0,Size=UDim2.new(1,0,0,thickness),Position=UDim2.new(0,0,0,0),Parent=frame})
	end
	if bottom then
		utility.new("Frame",{Name="_bBottom",BackgroundColor3=color,BorderSizePixel=0,Size=UDim2.new(1,0,0,thickness),Position=UDim2.new(0,0,1,-thickness),Parent=frame})
	end
	if left then
		utility.new("Frame",{Name="_bLeft",BackgroundColor3=color,BorderSizePixel=0,Size=UDim2.new(0,thickness,1,0),Position=UDim2.new(0,0,0,0),Parent=frame})
	end
	if right then
		utility.new("Frame",{Name="_bRight",BackgroundColor3=color,BorderSizePixel=0,Size=UDim2.new(0,thickness,1,0),Position=UDim2.new(1,-thickness,0,0),Parent=frame})
	end
end

local function cornerAndBorder(parent, rounded, radius, borderOpts, defaultBorderColor)
	applyCorner(parent, rounded, radius)
	applyBorder(parent, borderOpts, defaultBorderColor)
end
...
function comet:new(props)
	...
	local roundness = props.roundness or 1
	local windowRounding = props.rounding and props.rounding.window or roundness
	local windowRounded = props.rounding and (props.rounding.window ~= nil) and (props.rounding.window) or rounded
	local borderOpts = props.border or {}
	local sectionWidth = props.sectionWidth
	local sectionGap = props.sectionGap
	local window = {}
	...
	cornerAndBorder(outline, windowRounded, windowRounding, borderOpts, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(outline2, windowRounded, windowRounding, borderOpts, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(indent, windowRounded, windowRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(main, windowRounded, windowRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(outline3, windowRounded, windowRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(holder, windowRounded, windowRounding, {}, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(holder2, windowRounded, windowRounding, {}, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(tabs, windowRounded, windowRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(outline4, windowRounded, windowRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	utility.new(
		"UIListLayout",
		{
...
function comet:watermark()
	...
	cornerAndBorder(outline, self.rounded, self.rounding.window or self.roundness, self.border, Color3.fromRGB(12,12,12))
	...
	cornerAndBorder(outline2, self.rounded, self.rounding.window or self.roundness, self.border, Color3.fromRGB(12,12,12))
	local indent = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,0.5),
	...
	cornerAndBorder(indent, self.rounded, self.rounding.window or self.roundness, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local title = utility.new(
		"TextLabel",
		{
			AnchorPoint = Vector2.new(0.5,0),
...
function comet:loader(props)
	...
	local rounded = props.rounded
	if rounded == nil then rounded = self.rounded end
	local roundness = props.roundness or self.roundness
	local lr = props.rounding and props.rounding.loader or roundness
	local lrnd = props.rounding and (props.rounding.loader ~= nil) and (props.rounding.loader) or rounded
	local borderOpts = props.border or self.border
	local loader = {}
	local screen = utility.new(
		"ScreenGui",
		{
	...
	cornerAndBorder(outline, lrnd, lr, borderOpts, Color3.fromRGB(12,12,12))
	local outline2 = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,0.5),
	...
	cornerAndBorder(outline2, lrnd, lr, borderOpts, Color3.fromRGB(12,12,12))
	local indent = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,0.5),
	...
	cornerAndBorder(indent, lrnd, lr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local title = utility.new(
		"TextLabel",
		{
			AnchorPoint = Vector2.new(0.5, 0),
	...
	local function makebutton(nameb, parent)
		...
		cornerAndBorder(button_holder, lrnd, lr, {}, Color3.fromRGB(12,12,12))
		...
		cornerAndBorder(button_outline, lrnd, lr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
		...
		cornerAndBorder(button_outline2, lrnd, lr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
...
function comet:page(props)
	...
	local tabRounding = props.rounding and props.rounding.tab or (self.rounding and self.rounding.window or self.roundness)
	local tabRounded = props.rounded
	if tabRounded == nil then tabRounded = self.rounded end
	local borderOpts = props.border or self.border
	local page = {}
	...
	cornerAndBorder(tabbutton, tabRounded, tabRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	cornerAndBorder(outline, tabRounded, tabRounding, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(pageholder, tabRounded, tabRounding, {}, Color3.fromRGB(12,12,12))
	...
	local ll = utility.new(
		"UIListLayout",
		{
			FillDirection = "Vertical",
			Padding = UDim.new(0, self.layout.sectionGap or 10),
			Parent = left
		}
	)
...
function pages:section(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.comet.rounded
	local roundness = props.roundness or self.comet.roundness
	local sr = props.rounding and props.rounding.section or roundness
	local srounded = props.rounding and (props.rounding.section ~= nil) and props.rounding.section or rounded
	local borderOpts = props.border or self.comet.border
	local layoutWidth = props.width or self.comet.layout.sectionWidth
	local layoutGap = (props.gap ~= nil) and props.gap or self.comet.layout.sectionGap
	if layoutGap == nil then layoutGap = 10 end
	side = side:lower()
	local parentFrame = self[side]
	if side == "both" then
		parentFrame = self.page
	end
	local section = {}
	...
	cornerAndBorder(sectionholder, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	if parentFrame:FindFirstChildOfClass("UIListLayout") then
		parentFrame:FindFirstChildOfClass("UIListLayout").Padding = UDim.new(0, layoutGap)
	end
	if layoutWidth then
		if side == "left" then
			self.left.Size = UDim2.new(0, layoutWidth, 1, 0)
			self.right.Position = UDim2.new(0, layoutWidth + 10, 0, 0)
			self.right.Size = UDim2.new(1, -(layoutWidth + 10), 1, 0)
		elseif side == "right" then
			self.right.Size = UDim2.new(0, layoutWidth, 1, 0)
			self.left.Size = UDim2.new(1, -(layoutWidth + 10), 1, 0)
		elseif side == "both" then
			sectionholder.Size = UDim2.new(1,0,0,size)
		end
	end
	local outline = utility.new(
		"Frame",
	...
	cornerAndBorder(outline, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
...
function pages:multisection(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.comet.rounded
	local roundness = props.roundness or self.comet.roundness
	local msr = props.rounding and props.rounding.multisection or roundness
	local msrounded = props.rounding and (props.rounding.multisection ~= nil) and props.rounding.multisection or rounded
	local borderOpts = props.border or self.comet.border
	local layoutWidth = props.width or self.comet.layout.sectionWidth
	local layoutGap = (props.gap ~= nil) and props.gap or self.comet.layout.sectionGap
	if layoutGap == nil then layoutGap = 10 end
	...
	cornerAndBorder(sectionholder, msrounded, msr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	if parentFrame:FindFirstChildOfClass("UIListLayout") then
		parentFrame:FindFirstChildOfClass("UIListLayout").Padding = UDim.new(0, layoutGap)
	end
	if layoutWidth then
	...
	cornerAndBorder(outline, msrounded, msr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local color = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,0),
	...
	local buttons = utility.new(
		"Frame",
		{
			...
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
			Size = UDim2.new(1,-6,0, notitle and 0 or 20),
			Position = UDim2.new(0.5,0,0, notitle and 0 or 5),
			Parent = tabsholder
		}
	)
	local tabs = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,1),
			BackgroundColor3 = Color3.fromRGB(20, 20, 20),
			Size = UDim2.new(1,-6,1, notitle and -5 or -27),
			Position = UDim2.new(0.5,0,1, notitle and -3 or -3),
			Parent = tabsholder
		}
	)
	cornerAndBorder(tabs, msrounded, msr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	utility.new(
		"UIListLayout",
		{
			FillDirection = "Horizontal",
	...
	cornerAndBorder(tabs_outline, msrounded, msr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	multisection = {
		["comet"] = self.comet,
		["sectionholder"] = sectionholder,
		["color"] = color,
	...
	if titleObj then
		self.comet.labels[#self.comet.labels+1] = titleObj
	end
...
function multisections:section(props)
	local name = props.name or props.Name or props.page or props.Page or props.pagename or props.Pagename or props.PageName or props.pageName or "new ui"
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded
	local roundness = props.roundness or self.roundness
	local sr = props.rounding and props.rounding.msTab or roundness
	local srounded = props.rounding and (props.rounding.msTab ~= nil) and props.rounding.msTab or rounded
	local mssection = {}
	local tabbutton = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(20, 20, 20),
			Size = UDim2.new(0,60,0, math.max(20, self.notitle and 20 or 20)),
			Parent = self.buttons
		}
	)
	cornerAndBorder(tabbutton, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(20, 20, 20),
	...
	cornerAndBorder(outline, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local button = utility.new(
		"TextButton",
		{
			AnchorPoint = Vector2.new(0,0),
	...
	local line = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(20, 20, 20),
			BorderSizePixel = 0,
			Size = UDim2.new(1,0,0, self.notitle and 3 or 2),
			Position = UDim2.new(0,0,1, self.notitle and -3 or 0),
			ZIndex = 2,
			Parent = outline
		}
	)
	...
	if isFirst then
		...
		mssection.line.Size = UDim2.new(1,0,0, self.notitle and 3 or 3)
			...
			for i,v in pairs(self.mssections) do
				if v ~= mssection then
					if v.open then
						...
						v.line.Size = UDim2.new(1,0,0, self.notitle and 2 or 2)
...
function sections:toggle(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local tr = props.rounding and props.rounding.toggle or roundness
	local trounded = props.rounding and (props.rounding.toggle ~= nil) and props.rounding.toggle or rounded
	local borderOpts = props.border or self.comet.border
	local requires = props.requires or {}
	local parentToggle = props.parent or nil
	local toggle = {}
	...
	cornerAndBorder(outline, trounded, tr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local button = utility.new(
		"TextButton",
		{
			AnchorPoint = Vector2.new(0,0),
	...
	cornerAndBorder(color, trounded, tr, {border=false}, Color3.fromRGB(56,56,56))
	utility.new(
		"UIGradient",
		{
			Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},
	...
	toggle = {
		...
		["requires"] = requires,
		["parent"] = parentToggle,
		["isSub"] = parentToggle and true or false
	}
	if def then
		table.insert(self.comet.themeitems["accent"]["BackgroundColor3"],color)
	end
	local function canEnable()
		if not toggle.requires or #toggle.requires == 0 then return true end
		for _,key in ipairs(toggle.requires) do
			local p = self.pointers and self.pointers[key]
			if p and p.current == false then
				return false
			end
		end
		return true
	end
	local function setVisualDisabled(disabled)
		if disabled then
	...
	setVisualDisabled(not canEnable() and def)
	button.MouseButton1Down:Connect(function()
	...
	if toggle.isSub and toggle.parent and toggle.parent.toggleholder then
		toggle.toggleholder.Size = UDim2.new(1,-10,0,15)
		toggle.toggleholder.Position = UDim2.new(0,10,0,0)
	end
	local pointer = props.pointer or props.Pointer or props.pointername or props.Pointername or props.PointerName or props.pointerName or nil
	if pointer then
		if self.pointers then
...
function toggles:set(bool)
	if bool ~= nil then
		...
		local function canEnable()
			...
			for _,key in ipairs(toggle.requires) do
				local p = nil
				for k,v in pairs(toggle.comet.pointers) do
					if v[key] then p = v[key] break end
				end
				if not p then
					p = toggle.comet.pointers[key]
				end
				if p and p.current == false then
					return false
				end
		...
		if bool and not canEnable() then
			return
		end
...
function sections:button(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local br = props.rounding and props.rounding.button or roundness
	local brounded = props.rounding and (props.rounding.button ~= nil) and props.rounding.button or rounded
	local borderOpts = props.border or self.comet.border
	local button = {}
	...
	cornerAndBorder(outline, brounded, br, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline2 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	cornerAndBorder(outline2, brounded, br, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local color = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
...
function sections:slider(props)
	...
	local rounding = props.roundingValue or props.RoundingValue or props.round or props.Round or props.decimals or props.Decimals or false
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local sr = props.rounding and props.rounding.slider or roundness
	local srounded = props.rounding and (props.rounding.slider ~= nil) and props.rounding.slider or rounded
	local borderOpts = props.border or self.comet.border
	...
	local function slidefn()
		local sizepx = math.clamp(plr:GetMouse().X - slider.color.AbsolutePosition.X ,0 ,slider.color.AbsoluteSize.X)
		local result = (slider.max - slider.min) / slider.color.AbsoluteSize.X * sizepx + slider.min
		if slider.roundingVal then
			...
			if slider.tick then
				...
				slider.slide:TweenSize(UDim2.new((1 / slider.color.AbsoluteSize.X) * sizepx ,0 ,1 ,0) ,Enum.EasingDirection.Out ,Enum.EasingStyle.Quad ,0.15 ,true)
			end
		else
...
function sections:dropdown(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local dr = props.rounding and props.rounding.dropdown or roundness
	local drounded = props.rounding and (props.rounding.dropdown ~= nil) and props.rounding.dropdown or rounded
	local borderOpts = props.border or self.comet.border
	local dropdown = {}
	...
	cornerAndBorder(outline, drounded, dr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline2 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	cornerAndBorder(outline2, drounded, dr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local color = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
...
function sections:buttonbox(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local br = props.rounding and props.rounding.buttonbox or roundness
	local brounded = props.rounding and (props.rounding.buttonbox ~= nil) and props.rounding.buttonbox or rounded
	local borderOpts = props.border or self.comet.border
	local buttonbox = {}
...
function sections:multibox(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local mr = props.rounding and props.rounding.multibox or roundness
	local mrounded = props.rounding and (props.rounding.multibox ~= nil) and props.rounding.multibox or rounded
	local borderOpts = props.border or self.comet.border
	local defstr = ""
	if #def > 1 then
	...
	cornerAndBorder(outline, mrounded, mr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline2 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	cornerAndBorder(outline2, mrounded, mr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local color = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
...
function sections:textbox(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local tr = props.rounding and props.rounding.textbox or roundness
	local trounded = props.rounding and (props.rounding.textbox ~= nil) and props.rounding.textbox or rounded
	local borderOpts = props.border or self.comet.border
	local textbox = {}
	...
	cornerAndBorder(outline2, trounded, tr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	local color = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
...
function sections:keybind(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local kr = props.rounding and props.rounding.keybind or roundness
	local krounded = props.rounding and (props.rounding.keybind ~= nil) and props.rounding.keybind or rounded
	local borderOpts = props.border or self.comet.border
	local default = ".."
	local typeis = nil
	...
	cornerAndBorder(outline, krounded, kr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline2 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
...
function sections:colorpicker(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local cr = props.rounding and props.rounding.colorpicker or roundness
	local crounded = props.rounding and (props.rounding.colorpicker ~= nil) and props.rounding.colorpicker or rounded
	local borderOpts = props.border or self.comet.border
	...
	cornerAndBorder(outline, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline2 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	cornerAndBorder(cpholder, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline2b = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
			Size = UDim2.new(1,0,1,0),
	...
	cornerAndBorder(outline2b, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(outline3, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(huepicker, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline4 = utility.new(
		"Frame",
		{
	...
	cornerAndBorder(outline4, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(huecursor, true, 2, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	...
	local function textbox(parent,size,position)
		...
		cornerAndBorder(outline5, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
		...
		cornerAndBorder(outline6, crounded, cr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	local red = textbox(outline2b,UDim2.new(0,62,0,20),UDim2.new(0,5,0,175))
	local green = textbox(outline2b,UDim2.new(0,62,0,20),UDim2.new(0,5,0,175))
	green[1].AnchorPoint = Vector2.new(0.5,0)
	green[1].Position = UDim2.new(0.5,0,0,175)
	local blue = textbox(outline2b,UDim2.new(0,62,0,20),UDim2.new(0,5,0,175))
	blue[1].AnchorPoint = Vector2.new(1,0)
	blue[1].Position = UDim2.new(1,-5,0,175)
	local hex = textbox(outline2b,UDim2.new(1,-10,0,20),UDim2.new(0,5,0,200))
	hex[2].Size = UDim2.new(1,-12,1,0)
	hex[2].TextXAlignment = "Left"
	local huecursor_inline = huecursor_inline
	colorpicker = {
		["comet"] = self.comet,
		["cpholder"] = cpholder,
		["cpcolor"] = cpcolor,
	...
	local function movehue()
		...
		colorpicker.huecursor_inline.BackgroundColor3 = Color3.fromHSV(resy,1,1)
		...
		colorpicker.huecursor:TweenPosition(UDim2.new(0.5,0,resy,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
	end
	local function movecp()
		...
		updateboxes()
		colorpicker.callback(colorpicker.current)
		colorpicker.cpcursor:TweenPosition(UDim2.new(resx,0,resy,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
	end
	button.MouseButton1Down:Connect(function()
...
function sections:selection(props)
	...
	local rounded = (props.rounded ~= nil) and props.rounded or self.rounded or self.comet.rounded
	local roundness = props.roundness or self.roundness or self.comet.roundness
	local sr = props.rounding and props.rounding.selection or roundness
	local srounded = props.rounding and (props.rounding.selection ~= nil) and props.rounding.selection or rounded
	local borderOpts = props.border or self.comet.border
	local selection = {}
	local items = {}
	local buttons = {}
	...
	cornerAndBorder(outline2, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	cornerAndBorder(itemsHolder, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
	local outline3 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	cornerAndBorder(outline3, srounded, sr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	local function createButton(propsb)
		...
		cornerAndBorder(btnOutline, self.rounded, self.roundness, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
		...
		cornerAndBorder(btnOutline2, self.rounded, self.roundness, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
	...
	function selection:addTextbox(propsb)
		...
		cornerAndBorder(outline5, self.rounded, self.roundness, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(12,12,12)}, Color3.fromRGB(12,12,12))
		local outline6 = utility.new(
			"Frame",
			{
				BackgroundColor3 = Color3.fromRGB(24, 24, 24),
		...
		cornerAndBorder(outline6, self.rounded, self.roundness, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
		local color2 = utility.new(
			"Frame",
			{
				AnchorPoint = Vector2.new(0, 0),
...
function comet:notify(props)
    ...
    local nr = props.rounding and props.rounding.notification or (self.rounding and self.rounding.window or self.roundness)
    local nrounded = props.rounded
    if nrounded == nil then nrounded = self.rounded end
    local borderOpts = props.border or self.border
    local notification = {}
    ...
    cornerAndBorder(notifHolder, nrounded, nr, borderOpts, Color3.fromRGB(12,12,12))
    local outline2 = utility.new(
        "Frame",
        {
            AnchorPoint = Vector2.new(0.5, 0.5),
    ...
    cornerAndBorder(outline2, nrounded, nr, borderOpts, Color3.fromRGB(12,12,12))
    local indent = utility.new(
        "Frame",
        {
            AnchorPoint = Vector2.new(0.5, 0.5),
    ...
    cornerAndBorder(indent, nrounded, nr, {borderTop=true,borderBottom=true,borderLeft=true,borderRight=true,borderColor=Color3.fromRGB(56,56,56)}, Color3.fromRGB(56,56,56))
    local titleLabel = utility.new(
        "TextLabel",
        {
            AnchorPoint = Vector2.new(0.5, 0),
