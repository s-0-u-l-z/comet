...
local function applyRoundness(instance, r)
    if not instance then return end
    local rounded = instance:FindFirstChildOfClass("UICorner")
    if not rounded then
        rounded = Instance.new("UICorner")
        rounded.Name = "UICorner_Managed"
        rounded.Parent = instance
    end
    rounded.CornerRadius = UDim.new(0, tonumber(r) or 0)
end

local function ensureRichText(labelOrBox)
    if not labelOrBox or not labelOrBox:IsA("TextLabel") and not labelOrBox:IsA("TextBox") then return end
    labelOrBox.RichText = true
end
...
function comet:new(props)
	...
	local roundness = tonumber(props.roundness or props.Roundness or props.corner or props.Corner or props.cornerRadius or props.CornerRadius) or 0
	...
	applyRoundness(outline, roundness)
	...
	applyRoundness(outline2, roundness)
	...
	applyRoundness(indent, roundness)
	...
	applyRoundness(outline3, roundness)
	...
	applyRoundness(outline4, roundness)
	...
	do
		local examplePage = window:page({name="Example"})
		examplePage:openpage()
		local leftSection = examplePage:section({name="Demo", side="left", size=130})
		leftSection:paragraph({
			title = "Strikethrough",
			text = "<s>This text is struck through</s>",
			titlesize = window.textsize,
			textsize = window.textsize
		})
		local rightSection = examplePage:section({name="Corners", side="right", size=90})
		rightSection:paragraph({
			title = "Rounded Corners",
			text = "Window roundness in pixels: "..tostring(window.roundness),
			titlesize = window.textsize,
			textsize = window.textsize
		})
		leftSection:paragraph({
			title = nil,
			text = "This is normal text, unaffected.",
			textsize = window.textsize
		})
	end
...
function comet:watermark()
	...
	applyRoundness(outline, self.roundness)
	...
	applyRoundness(outline2, self.roundness)
	local indent = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,0.5),
	...
	applyRoundness(indent, self.roundness)
	local title = utility.new(
		"TextLabel",
		{
			AnchorPoint = Vector2.new(0.5,0),
...
function comet:loader(props)
	...
	local makebutton = function(name, parent)
		...
		applyRoundness(button_outline, self.roundness)
		...
		applyRoundness(button_outline2, self.roundness)
		...
		applyRoundness(button_color, self.roundness)
		utility.new(
			"UIGradient",
			{
...
function comet:setroundness(pixels)
	self.roundness = tonumber(pixels) or 0
	applyRoundness(self.outline, self.roundness)
	for _,desc in ipairs(self.outline:GetDescendants()) do
		if desc:IsA("Frame") or desc:IsA("ScrollingFrame") or desc:IsA("TextButton") or desc:IsA("ImageButton") then
			applyRoundness(desc, self.roundness)
		end
	end
end
...
function comet:page(props)
	...
	applyRoundness(tabbutton, self.roundness)
	local outline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
...
function pages:section(props)
	...
	applyRoundness(sectionholder, self.comet.roundness)
	local outline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	applyRoundness(outline, self.comet.roundness)
...
function pages:multisection(props)
	...
	applyRoundness(tabs, self.comet.roundness)
	utility.new(
		"UIListLayout",
		{
			FillDirection = "Horizontal",
	...
	applyRoundness(tabs_outline, self.comet.roundness)
	multisection = {
		["comet"] = self.comet,
		["sectionholder"] = sectionholder,
		["color"] = color,
...
function multisections:section(props)
	...
	applyRoundness(tabbutton, self.comet.roundness)
	local outline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(20, 20, 20),
...
function sections:toggle(props)
	...
	applyRoundness(color, self.comet.roundness)
	if def then
		table.insert(self.comet.themeitems["accent"]["BackgroundColor3"],color)
	end
	utility.new(
...
function sections:button(props)
	...
	applyRoundness(outline2, self.comet.roundness)
	local color = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(30, 30, 30),
...
function sections:slider(props)
	...
	applyRoundness(slide, self.comet.roundness)
...
function sections:dropdown(props)
	...
	applyRoundness(optionsholder, self.comet.roundness)
	local size = #options
	...
	applyRoundness(optionsoutline, self.comet.roundness)
	utility.new(
		"UIListLayout",
		{
			FillDirection = "Vertical",
...
function sections:colorpicker(props)
	...
	applyRoundness(outline2a, self.comet.roundness)
	...
	applyRoundness(cpcolor, self.comet.roundness)
	utility.new(
		"UIGradient",
		{
			Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},
	...
	applyRoundness(cpholder, self.comet.roundness)
	local outline2 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	applyRoundness(cpholder2, self.comet.roundness)
	local outline3 = utility.new(
		"Frame",
		{
	...
	applyRoundness(outline3, self.comet.roundness)
	...
	applyRoundness(huepicker, self.comet.roundness)
	local outline4 = utility.new(
		"Frame",
		{
	...
	applyRoundness(outline4, self.comet.roundness)
	...
	local function textbox(parent,size,position)
		...
		applyRoundness(outline5, self.comet.roundness)
		...
		applyRoundness(outline6, self.comet.roundness)
		...
		applyRoundness(color2, self.comet.roundness)
		utility.new(
			"UIGradient",
			{
				Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},
...
function sections:selection(props)
	...
	applyRoundness(itemsHolder, self.comet.roundness)
	local outline3 = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromRGB(24, 24, 24),
	...
	local function createButton(props)
		...
		applyRoundness(btnOutline, self.comet.roundness)
		...
		applyRoundness(btnOutline2, self.comet.roundness)
		...
		applyRoundness(btnColor, self.comet.roundness)
		utility.new(
			"UIGradient",
			{
...
function comet:notify(props)
	...
	applyRoundness(notifHolder, self.roundness)
    local outline2 = utility.new(
        "Frame",
        {
            AnchorPoint = Vector2.new(0.5, 0.5),
...
function sections:title(props)
	...
	function titleT:set(newtext)
		titlelabel.Text = newtext
	end
	function titleT:setcolor(color)
